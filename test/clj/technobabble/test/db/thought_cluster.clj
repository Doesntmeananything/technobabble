(ns technobabble.test.db.thought-cluster
  (:require [clojure.string :refer [split-lines split]]
            [clojure.set :refer [intersection]]
            [clojure.test :refer :all]
            [technobabble.db.core :refer [*db*] :as db]
            [technobabble.db.memory :as memory]
            [technobabble.db.thought-cluster :as tc]
            [technobabble.db.user :as user]
            [technobabble.test.db.user :as tdu]
            [mount.core :as mount]
            [technobabble.test.db.memory :as tdm])
...
(deftest test-remove-thought
  ...
  (testing "We can remove a thought from a cluster"
    (let [query     (memory/query tdu/ph-username)
      ...
      ;;
      ;; See https://gitlab.com/Numergent/technobabble/issues/112#note_63788223
