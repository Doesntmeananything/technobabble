...
(reg-event-db
 :auth-request
 (fn [app-state [_ register?]]
   (let [url     (if register? "register" "login")
         to-send (if register? [:username :password :password-confirm] [:username :password])
          ;; Should probably centralize password validation, so we can use the same function
          ;; both here and when the UI is being updated
         valid?  (or (not register?)
                     (= (get-in app-state [:credentials :password])
                        (get-in app-state [:credentials :password-confirm])))]
     (when valid?
...
(reg-event-db
 ...
 (fn [app-state [_ token]]
   ...
   (cond
     ...
     (= :register (get-in app-state [:ui-state :section])) (dispatch [:state-ui-section :record])
   ...
   (-> app-state
       ...
       (assoc-in [:credentials :password-confirm] nil))))
